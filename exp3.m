...
plot(s,'g','LineWidth',2); 
title('Sine Wave'); 
d1=designfilt('lowpassfir','FilterOrder',20,'CutoffFrequency',3,'SampleRate',fs); 
y=filter(d1,x); 
subplot(4,1,2) 
...
d1=designfilt('lowpassfir','FilterOrder',22,'CutoffFrequency',3,'SampleRate',fs); 
y=filter(d1,x); 
y1=filtfilt(d1,x); 
d2=designfilt('lowpassfir','FilterOrder',15,'CutoffFrequency',3,'SampleRate',fs); 
y=filter(d2,x); 
y2=filtfilt(d2,x); 
d3=designfilt('lowpassfir','FilterOrder',28,'CutoffFrequency',3,'SampleRate',fs); 
y=filter(d3,x); 
y3=filtfilt(d3,x); 
figure(3) 
subplot(4,1,1); 
